...
static int ble_on_disc_chrs(uint16_t conn_handle, const struct ble_gatt_error *error,
                            const struct ble_gatt_chr *chr, void *arg);
static int ble_on_write_cccd(uint16_t conn_handle, const struct ble_gatt_error *error,
...
static void subscribe_to_notifications(uint16_t conn_handle, uint16_t val_handle) {
  ...
  int rc = ble_gattc_write_flat(conn_handle, val_handle + 1, &cccd_val, sizeof(cccd_val),
                                ble_on_write_cccd, NULL);
  if (rc != 0) {
...
static int ble_on_disc_svc(uint16_t conn_handle, const struct ble_gatt_error *error,
  ...
  if (error->status == 0) {
    ESP_LOGI(TAG, "Discovered NUS Service: start_handle=%d, end_handle=%d",
             service->start_handle, service->end_handle);
    ble_gattc_disc_all_chrs(conn_handle, service->start_handle, service->end_handle,
                            ble_on_disc_chrs, NULL);
  } else if (error->status == BLE_HS_EDONE) {
        ...
        if (data_buf) {
          ...
          if (rc == 0) {
            if (registered_recv_cb) {
              ...
              registered_recv_cb(peer_mac, data_buf, data_len, peer_addr_type, -50);
            }
          }
          free(data_buf);
        }
...
static void start_scan(void) {
  if (!is_initialized || !is_synced) return;
...
static esp_err_t ble_driver_send(const uint8_t *peer_mac, const uint8_t *data, size_t len) {
  ...
  int rc = ble_gattc_write_no_rsp_flat(ble_conn_handle, nus_tx_handle, data, len);
  if (rc != 0) {
...
static esp_err_t ble_driver_connect_peer(const uint8_t *peer_mac, uint8_t channel) {
  ...
  ESP_LOGI(TAG, "BLE connecting to peer: %02X:%02X:%02X:%02X:%02X:%02X",
